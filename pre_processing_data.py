...
if __name__ == '__main__':
    ...
    #         fp.write(sentence + "\n")
    #     sentence = f.readline()
    data_file = "./data/chatbot_dataset.txt"
    data_converted_file = "./data/data_converted.txt"
    f = open(data_file, encoding='utf8') 
    sentence = f.readline()
    current_line = 0
    # print(chuan_hoa_dau_cau_tieng_viet(sentence))
    while sentence:
        current_line += 1
        if current_line % 100 == 0:
            print(sentence)

        new_sentence = chuan_hoa_dau_cau_tieng_viet(sentence)  
        with open(data_converted_file, 'a+', encoding='utf8') as fp:
                fp.write(new_sentence + "\n")
